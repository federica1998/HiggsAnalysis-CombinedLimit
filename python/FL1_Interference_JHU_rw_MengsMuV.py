...
class FL1_Interference_JHU_rw_MengsMuV(PhysicsModel):
    def __init__(self):
        self.altSignal = "ALT_L1"
...
#        self.modelBuilder.out.var("muf").setAttribute("flatParam")
        ...
        self.modelBuilder.doVar('expr::a3("(@0>0 ? 1 : -1) * sqrt(abs(@0)*{sigma1_HZZ}/{sigmaL1_HZZ})", CMS_zz4l_fai1)'.format(**xsecs))

        self.modelBuilder.factory_('expr::muVc("@1/(1+500.*abs(@0))", CMS_zz4l_fai1,muV)'.format(**xsecs))
        self.modelBuilder.factory_('expr::smCoupling_VBF("@0*@1**2 - @0*@1*@2*sqrt({sigmaL1_VBF}/{sigma1_VBF})", muVc,a1,a3)'.format(**xsecs))
        self.modelBuilder.factory_('expr::smCoupling_ZH("@0*@1**2 - @0*@1*@2*sqrt({sigmaL1_ZH}/{sigma1_ZH})", muVc,a1,a3)'.format(**xsecs))
        self.modelBuilder.factory_('expr::smCoupling_WH("@0*@1**2 - @0*@1*@2*sqrt({sigmaL1_WH}/{sigma1_WH})", muVc,a1,a3)'.format(**xsecs))


        self.modelBuilder.factory_('expr::bsmCoupling_VBF("@0*@1**2*{sigmaL1_VBF}/{sigma1_VBF} - @0*@1*@2*sqrt({sigmaL1_VBF}/{sigma1_VBF})", muVc,a3,a1)'.format(**xsecs))
        self.modelBuilder.factory_('expr::bsmCoupling_ZH("@0*@1**2*{sigmaL1_ZH}/{sigma1_ZH} - @0*@1*@2*sqrt({sigmaL1_ZH}/{sigma1_ZH})", muVc,a3,a1)'.format(**xsecs))
        self.modelBuilder.factory_('expr::bsmCoupling_WH("@0*@1**2*{sigmaL1_WH}/{sigma1_WH} - @0*@1*@2*sqrt({sigmaL1_WH}/{sigma1_WH})", muVc,a3,a1)'.format(**xsecs))


        self.modelBuilder.factory_('expr::intCoupling_VBF("@0*@1*@2*sqrt({sigmaL1_VBF}/{sigma1_VBF})*{sigmaa1L1int_VBF}/{sigma1_VBF}", muVc,a1,a3)'.format(**xsecs))
        self.modelBuilder.factory_('expr::intCoupling_ZH("@0*@1*@2*sqrt({sigmaL1_ZH}/{sigma1_ZH})*{sigmaa1L1int_ZH}/{sigma1_ZH}", muVc,a1,a3)'.format(**xsecs))
        self.modelBuilder.factory_('expr::intCoupling_WH("@0*@1*@2*sqrt({sigmaL1_WH}/{sigma1_WH})*{sigmaa1L1int_WH}/{sigma1_WH}", muVc,a1,a3)'.format(**xsecs))
...
FL1_Interference_JHU_rw_MengsMuV = FL1_Interference_JHU_rw_MengsMuV()
